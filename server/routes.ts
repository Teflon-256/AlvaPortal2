...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Generate current 2FA code (for testing/demo purposes)
  app.get('/api/generate-2fa', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      ...
      res.json({
        code: token,
        timeRemaining,
        message: "Current TOTP code (for testing only)"
      });
    } catch (error) {
  ...
  // Verify 2FA code (for testing/demo purposes)
  app.post('/api/verify-2fa', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { code } = req.body;
      ...
      res.json({
        valid: verified,
        message: verified ? "Code is valid!" : "Invalid code. Please try again."
      });
    } catch (error) {
      console.error("Error verifying 2FA code:", error);
      res.status(500).json({ message: "Failed to verify code", valid: false });
    }
  });
